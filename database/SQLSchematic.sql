...
CREATE TABLE `users` (
  ...
  `password_hash` varchar(255) NOT NULL COMMENT 'Store secure password hash using PHP password_hash()',
  `role` varchar(20) NOT NULL COMMENT 'e.g., student, admin',
...
ALTER TABLE `grades`
  ...
  ADD UNIQUE KEY `UQ_StudentSubjectTerm` (`student_id`,`subject_id`,`semester_id`,`school_year_id`),
  ADD KEY `subject_id` (`subject_id`),
...
ALTER TABLE `curriculum`
  ...
  ADD CONSTRAINT `curriculum_ibfk_2` FOREIGN KEY (`subject_id`) REFERENCES `subjects` (`subject_id`);
...
ALTER TABLE `grades`
  ...
  ADD CONSTRAINT `grades_ibfk_4` FOREIGN KEY (`school_year_id`) REFERENCES `school_years` (`school_year_id`);
