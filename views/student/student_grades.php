...
(function() {
    ...
    async function init() {
        try {
            ...
            const histRes = await fetch('<?= APP_URL ?>/student/api/grades/history');
            const histData = await histRes.json();
            scholasticHistory = histData.data || [];
            updateOverallSummary();

            // Fetch Terms
            const termsRes = await fetch('<?= APP_URL ?>/student/api/grades/terms');
    ...
    async function loadTermData() {
        ...
        try {
            const res = await fetch(`<?= APP_URL ?>/student/api/grades/term?school_year=${encodeURIComponent(year)}&semester=${encodeURIComponent(sem)}`);
    ...
    document.getElementById('tab-progress').addEventListener('click', async function() {
        ...
        try {
            const res = await fetch('<?= APP_URL ?>/student/api/grades/progress');
            const data = await res.json();
    ...
    document.getElementById('exportPdfBtn').addEventListener('click', function() {
        ...
        const exportUrl = new URL('<?= APP_URL ?>/student/printables/academic-record', window.location.origin);
    ...
    document.getElementById('exportCurriculumPdfBtn').addEventListener('click', function() {
        const exportUrl = new URL('<?= APP_URL ?>/student/printables/curriculum-progress', window.location.origin);
        exportUrl.searchParams.set('return_to', '/student/dashboard?view=get_student_grades');
        window.location.href = exportUrl.toString();
    });
